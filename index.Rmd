        ...
        ggplotly(
          ggplot(df,) +
            geom_line(aes(x = date, y = flow), 
                      lwd = 0.9, color = "#4f77e3") +
            theme_bw() +
            geom_hline(yintercept = df$range_upper_bound[1], lwd = 0.5) +
            geom_hline(yintercept = df$range_lower_bound[1], lwd = 0.5) +
            labs(
              y = "Flow",
              x = "Date",
              title = usgs_name())
...
This tab shows the relationship between boatable days and common drought indices PDSI, SPEI, and SPI. Users can drag the slider below to show a predicted number of annual boatable days given a specific drought index value.
...
shinyBS::bsPopover(
  ...
  content   = "This is the number of boatable days predicted by the linear regression equation, given the drought index value indicated by the position of the slider tool in the left panel.",
  placement = "top",
  options   = list(container = "body")
  )
tags$style(HTML("
