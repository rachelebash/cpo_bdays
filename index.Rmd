...
# TODO:
# Angus TODO list:
# - updateSliderInput - still no luck in dynamically changing the range of the slider based on a reactive variable. 
  # Lines 345-352 is the best attempt at it, but there are other attempts below that. (DONE)

# - it's not crucial, so I don't want to spend much time on it if it is complicated, 
  # but I wanted to change the color of the marker on the map when you click on it. no worries if that doesn't work out (DONE)
...
```{r}
# - updateSliderInput - still no luck in dynamically changing the range of the slider based on a reactive variable. 
  # Lines 345-352 is the best attempt at it, but there are other attempts below that.

# - it's not crucial, so I don't want to spend much time on it if it is complicated, 
  # but I wanted to change the color of the marker on the map when you click on it. no worries if that doesn't work out
